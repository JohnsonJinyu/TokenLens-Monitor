...
import type { BalanceInfo, ProviderConfig, UsageEntry } from '../providers/base';
import { UsageTracker, GlobalStats } from '../tracker/usageTracker';
import { StorageManager } from '../tracker/storage';
import { getPricing, getProviders } from '../config/settings';
...
interface DashboardSnapshot {
  ...
  providerRows: Array<{
    ...
    capabilityTags?: Array<{ label: string; enabled: boolean }>;
...
export class DashboardPanel implements vscode.WebviewViewProvider {
  ...
  private buildSnapshot(stats: GlobalStats): DashboardSnapshot {
    ...
    for (const providerConfig of getProviders()) {
  ...
  private buildProviderSettings(settings: Record<string, any>): Array<Record<string, any>> {
    const providers = getProviders();
    const fallbackProviders: ProviderConfig[] = providers.length ? providers : [{
      id: 'deepseek',
      name: 'DeepSeek',
  ...
  private getProviderConfigMap(): Map<string, ProviderConfig> {
  ...
  private formatCapabilities(provider?: ProviderConfig): string {
  ...
  private getCapabilityTags(provider?: ProviderConfig): Array<{ label: string; enabled: boolean }> {
    const c = provider?.capabilities;
    return [
  ...
  private getProviderCapabilityState(provider?: ProviderConfig): string {
    if (!provider) { return '能力待声明'; }
    const c = provider.capabilities;
  ...
  private maskSecret(value: unknown): string {
