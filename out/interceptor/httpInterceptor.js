...
// 公开 API
// ============================================================
function startInterception(opts) {
    if (interceptEnabled) {
        return;
    }
    ...
    httpsModule.request = interceptor;
    // 同时替换 https.get（以防某些库直接调用 get）
    httpsModule.get = function (...args) {
        const req = interceptor.apply(this, args);
        req.end();
        return req;
    };
...
function createInterceptor() {
    return function (...args) {
        ...
        const hostname = urlOrOpts.hostname || urlOrOpts.host || '';
        const pathname = urlOrOpts.pathname || urlOrOpts.path || '';
        // ---- 检查是否是需要拦截的请求 ----
...
// ============================================================
function extractUsage(body, model, hostname, pathname) {
    try {
        // 尝试直接 JSON 解析（非流式响应）
...
function extractStreamUsage(body, model, hostname, pathname) {
...
function buildEntry(response, model, hostname, pathname) {
    ...
    else if (hostname.includes('siliconflow')) {
        provider = 'SiliconFlow';
    }
    else if (hostname.includes('moonshot')) {
        provider = 'Moonshot';
    }
    else if (hostname.includes('bigmodel')) {
        provider = 'Zhipu';
    }
    else if (hostname.includes('dashscope')) {
        provider = 'Qwen';
    }
    // 计算费用
...
// 费用计算
// ============================================================
function calculateCost(model, promptTokens, completionTokens, cacheHitTokens) {
    // 先查用户自定义定价，再查默认定价
    let pricing = customPricing[model];
    if (!pricing) {
    ...
    if (!pricing) {
        ...
        // 默认: 输入 $0.5/M, 输出 $2/M
        const inputCost = (promptTokens / 1000000) * 0.5;
        const outputCost = (completionTokens / 1000000) * 2;
        return inputCost + outputCost;
    }
    let inputCost = (promptTokens / 1000000) * pricing.input;
    // 缓存命中折扣
    if (cacheHitTokens > 0 && pricing.cacheHitDiscount) {
        const hitCost = (cacheHitTokens / 1000000) * pricing.input * pricing.cacheHitDiscount;
        const missCost = ((promptTokens - cacheHitTokens) / 1000000) * pricing.input;
